...
from home.models import Task, Review
...
class TestUser(TestCase):
    ...
    def test_get_user_not_present(self):
        response = self.client.get(reverse('user', args=[68]))
    ...
    def test_post_user_update_some_fields(self):
        response = self.client.post(reverse('user', args=[1]), {"fname": "New", "lname":"Name"})
        resp_json = json.loads((response.content).decode("utf-8"))
        self.assertEquals(resp_json, {
                "id": 1,
                "fname": "New",
    ...
    def test_post_user_update_all_fields(self):
        response = self.client.post(reverse('user', args=[1]), {"fname": "New", "lname": "Name", "email": "new@gmail.com", "bio":"I make changes", "pw":"secret", "location":"Virginia"})
        resp_json = json.loads((response.content).decode("utf-8"))
        self.assertEquals(resp_json, {
                "id": 1,
                "fname": "New",
